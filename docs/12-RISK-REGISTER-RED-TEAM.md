...
| R17 | T2 punch labels (jab/cross/hook/uppercut from stance+geometry heuristics) mislabel under camera-facing angles or depth noise, eroding trust in the mix/notation | 3×3 | Ambiguous straightness band stays null ("?"); vertical-dominance gate needs ≥15cm rise AND ≥1.2× travel; labels framed as heuristic until the trained classifier (E14) replaces them; M1 phone testing validates angles before the labels get marketing weight | core-wasm punch_class, [03](03-FEASIBILITY.md) |
| R18 | Drill scorecard pass/work verdicts flip on noise near the 450ms bar, teaching wrong lessons on single sessions | 2×3 | p75 (robust statistic) + ≥6-sample floor; verdicts framed as per-session with mastery requiring 3 consecutive passes; unmeasurable criteria render info/completion cards, never invented grades; thresholds re-validated at M1/M2 | web scorer.ts, content success_criteria |
| R19 | `?fastdrill=1` test hook is reachable in production (anyone can run toy-length drills) | 1×2 | Accepted: hook only shortens rounds; scoring floors still apply (short drills grade "too few to measure"), no data corruption possible. Revisit if drill history feeds rankings | web page.tsx fastDrillRef |
