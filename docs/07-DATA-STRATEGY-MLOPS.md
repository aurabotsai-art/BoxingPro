...
4. **Licensing posture (owner decision 2026-07-20, pragmatic setting)** — extracting *lessons* (techniques, thresholds, archetype parameters) from public video is unrestricted: ideas and facts aren't copyrightable. For *keypoint ingestion*, the owner accepts the pragmatic posture standard in ML research (Kinetics-class datasets are built from YouTube): ingest keypoints from any public registry video, CC-licensed preferred when equivalents exist but not required. Keypoint extraction retains zero expressive content (no pixels/audio/presentation — body geometry only), which is maximally transformative; residual risks are YouTube ToS (realistic ceiling: scraper/account blocking) and contested-but-favorable fair-use territory. Non-negotiables that keep the risk class low and cost nothing: **keypoints only, never retain or redistribute source video; every source stays in the ledger (`data/sources/`) with URL and access date — provenance diligence at fundraising/acquisition is a standard ask; never republish the dataset; never market "trained on [creator]'s videos."** Counsel sanity-checks this posture in the M7 consult (docs/13); if counsel flags a specific source class, the ledger makes removal mechanical.
