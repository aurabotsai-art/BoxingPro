...
| E2 | ✅ **v1 live in production** | `web/`: mirrored selfie view, live HUD (LIVE/tracking/fps pills), strike counter + punches/min, last-strike card (m/s + guard recovery + heuristic punch name incl. uppercuts), strike-beep + round bells (2:00/3:00 work, 1:00 rest), spoken + visual one-at-a-time cues (rest-suppressed), combos in boxing notation, punch-mix stats, deterministic post-session coach card (worst measured fault → content-library drill), guided drill sessions with spoken combo/direction callers for all 9 "App calls" protocols + structured conditioning intervals, drill scorecards graded against content success criteria, per-drill mastery streaks (3 consecutive passes = ★), Today's-plan suggestion chip, speed-trend chart, end-of-session summary + strike log + history + IDB archives + share card (punch mix + best combo), PWA/offline. E2E headless suite covers all of it incl. full drill lifecycle (?fastdrill hook). Deployed via GitHub→Vercel auto-deploy: https://boxing-pro.vercel.app | Nothing; M1 phone feedback tunes thresholds |
...
| E8 | ◐ **15 faults / 16 drills** (all 7 categories covered, linted — incl. a strict PyYAML validity pass — trainability-scored; telegraph_hand_dip + its two drills added alongside the live telegraph cue). Drills now ship into the app via `tools/gen_drill_lib.py` → `web/lib/session/drills.gen.ts` (CI checks snapshot freshness); every drill is browsable AND startable as a guided session | Grows toward ~40/~60 alongside classifier capability; remaining entries need coach-panel input to stay honest (M4) | M4 for depth |
...
| E13 | ◐ **Tier-2-lite chain complete** | `workers/deep_analysis/worker.py` --once: archive → CLI analysis → coach render, both schema-valid outputs written per job (verified on synthetic jab+hook). GPU re-pose and the Supabase poll loop remain | M6 to deploy; E11 for queue |
| E14 | ◐ **Pipeline dry-run green (3-class, in CI)** | tools/train/: extract_features.py (archive → analyze → labeled feature CSV; takes labeler exports or fixed labels) + train.py (logreg over Metrics Core features, k-fold report, sklearn-free portable model.json) + dry_run.sh (committed, CI-wired: jab/hook/uppercut × fps × height grid → 27 events → 5-fold 1.000 → 3-class model asserted; plumbing check — synthetic classes are trivially separable). Real training awaits labeled footage | M2 → E9 labeling |
